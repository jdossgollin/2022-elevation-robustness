...
%DIF ADD papers/earths-future/submission_02.tex   Mon Oct  3 12:56:09 2022
...
\begin{keypoints}
  ...
  \DIFaddbegin \item \DIFadd{Estimates of performance or robustness under deep uncertainty necessarily involve subjective judgement.
...
.
To ensure acceptable performance, current }\DIFaddend practice in engineering, infrastructure design, and \DIFdelbegin \DIFdel{local governance }\DIFdelend \DIFaddbegin \DIFadd{regulation }\DIFaddend relies heavily on standards that specify \DIFdelbegin \DIFdel{particular }\DIFdelend design events or conditions that buildings and infrastructure should safely withstand \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
\cite{kousky_voucher:2014} }\hskip0pt%DIFAUXCMD
or minimum elevations \mbox{%DIFAUXCMD
...
}\DIFdelend \DIFaddbegin \DIFadd{This provides a formal basis for, }\eg\DIFadd{, protecting hospitals and critical infrastructure to a higher degree than ordinary buildings \mbox{%DIFAUXCMD
\cite{asce_7-10:2013}}\hskip0pt%DIFAUXCMD
.
...
of climate hazards used in standard-setting and decision-making emphasize }\DIFdelend \DIFaddbegin \DIFadd{Moreover, estimates of the performance trade-offs require implicit or explicit assumptions about the likelihood of different possible futures.
...
Among several statistical assumptions these analyses make that have been recently called into question is stationarity \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
.
These deeply uncertain nonstationary hazards challenge not only the existing stationary estimates but, more fundamentally, the premise that objective estimates }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
...
\gls{dmdu} }\hskip0pt%DIFAUXCMD
emphasizes the value of identifying decisions }\DIFaddend that are robust\DIFaddbegin \DIFadd{, in some sense, to model limitations \mbox{%DIFAUXCMD
...
\DIFadd{In this paper we offer a first conceptual step towards bridging this divide by presenting a framework that combines the strengths of both approaches.
...
, we interpret these probability distributions not statements of fact, but rather as a }\DIFaddend self-consistent \DIFdelbegin \DIFdel{language with which to reason about the unknown rather than a statement of objective truth (see \mbox{%DIFAUXCMD
...
\DIFaddbegin \DIFadd{Many limitations of Uniform and other noninformative priors have been documented in the literature, including (i) that they can induce unrealistic (and non-Uniform) priors over functions of parameters and (ii) results can change when the parameterization of a given process is altered \mbox{%DIFAUXCMD
\cite{gupta_prior:2022,seaman_noninformative:2012}}\hskip0pt%DIFAUXCMD
.
}\DIFaddend 
...
.
}\DIFaddend We return in \cref{sec:results-conditional} to the challenge of decision making under multiple \glspl{pdf}.
...
\DIFdelbegin \DIFdel{The proposed scenario weighting }\DIFdelend \DIFaddbegin \DIFadd{xThe proposed \mbox{%DIFAUXCMD
\gls{sow} }\hskip0pt%DIFAUXCMD
