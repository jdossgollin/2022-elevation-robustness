...
# ╔═╡ baf8cff2-a42d-4bd0-a6c5-23fa3e00c96f
begin
    using PlutoUI
...
# ╔═╡ bc7cf4cd-2a7e-4393-a909-91dd7b985fbe
brick = get_norfolk_brick(syear = 2020; eyear = 2100);

# ╔═╡ 0e0f9d2c-6bbb-4b22-9e8d-b709a697c62d
n_chains = 4;

# ╔═╡ f6f87a43-8ab7-4fc5-9f32-91552f4ddca6
...
# ╔═╡ 7b3a9998-c830-4d19-b5f5-8f0256892dad
plot(fake_data_plot)

# ╔═╡ 56d871d2-e81b-4b88-b5f4-0bdd9ae7377c
savefig(fake_data_plot, plotsdir("fake_data.pdf"))
...
data_plot = plot(obs.year, y, label = "", ylabel = "Storm Surge (ft)", marker = ".");
...
begin
    ...
    hline!(
        predictive_plot,
        [quantile(yhat_prior_flat, 0.99)],
        color = colors[1],
        label = "Prior Q99",
    )
    hline!(
        predictive_plot,
        [quantile(yhat_flat, 0.99)],
        color = colors[2],
        label = "Posterior Q99",
    )
end
...
"Calculate an empirical return period"
...
function plot_return_period(obs::AnnualGageRecord, posterior::Chains; q = [0.1, 0.9])
    ...
    scatter!(p, return_periods, y, label = "Empirical: m/(N+1)")
    return p
end;
...
Now that we are relatively happy with our model, we can create a set of SOWs for future use

The first step is to draw samples of storm surge to go with our local sea level data
"""

# ╔═╡ 34645ff8-7e7d-4530-b94c-01c4964d192f
begin
    future_surges, _ = sample_predictive(posterior, length(brick[1].years))
    future_surges = future_surges[:] * 1u"ft"
end;

# ╔═╡ 3aa18c52-7465-4dfc-bbee-a9bb33dcc6c9
md"We this gives us a set of $(length(future_surges)) samples, each of length $(length(future_surges[1])) because it goes from $(first(brick[1].years)) to $(last(brick[1].years))."

# ╔═╡ 5b425c5f-e387-482f-9686-55c547467f4d
sows = SOWCollection(
    [
        StateOfWorld(first(brick).years, brk.lsl_m, srg) for
        (brk, srg) in zip(brick, future_surges)
    ],
    StatsBase.uweights(length(brick)),
);

# ╔═╡ 91371d4d-9d25-480b-be5a-fea90bb126f3
md"And save to file"

# ╔═╡ 577c21bb-41d1-4d82-a0bb-47df3c2a1502
DrWatson.wsave(datadir("processed", "SOWs.jld2"), Dict("sows" => sows))
