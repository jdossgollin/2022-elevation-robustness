...
We then apply this framework to understand exploratory modeling (\cref{sec:analysis-explore}) and scenario-conditional probabilistic analysis (\cref{sec:analysis-condition}).
In \cref{sec:analysis-synthesize} we discuss why synthesizing across multiple scenarios is necessary.
In \cref{sec:reweighting} we provide a formal method for synthesizing across scenarios.

It is common in climate risk management to work with an ensemble of trajectories of climate, economic, and other key variables, which we refer to as \glspl{sow}.
Often these \glspl{sow} are computationally expensive to generate, and thus it is important to use them efficiently.
...
We illustrate this concept in boxes (d) and (e) of \cref{fig:flowchart}, denoting the particular scenario $M_k$.
By treating the \glspl{sow} as \gls{iid} draws from $M_k$, the set of outcomes $u_{i, j}$ can be interpreted as \gls{iid} draws from the conditional distribution over outcomes, $p(u | x_i, M_k)$.
...
We can also characterize \gls{dmdu} methods that sample a set of parameters from fixed ranges as scenario-conditional analyses.
...
Scenario-conditional probabilistic analysis allows for uncertainty quantification and optimization, and is valuable in many copntexts.
...
In this section we provide a formal method for synthesizing across multiple scenarios to inform climate risk management.
...
Doing so requires a probabilistic model for this low-dimensional representation of the \glspl{sow},  $p(\psi)$.
We denote this model $p_\mathrm{belief}$ to emphasize that it represents a subjective belief about the \glspl{sow}, rather than an objectively verifiable choice.
...
We return in \cref{sec:results-conditional} to the challenge of decision making given multiple \glspl{pdf}.
...
We use a Gamma distribution for all three priors, parameterized following eq.~(S4).
