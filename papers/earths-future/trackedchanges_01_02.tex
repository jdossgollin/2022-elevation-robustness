...
%DIF ADD papers/earths-future/submission_02.tex   Mon Sep 26 08:25:50 2022
...
\begin{abstract}
  ...
  Some common approaches include analyzing a small subset of projections or invoking \DIFdelbegin \DIFdel{Laplace's principle of insufficient reason to justify a simple average}\DIFdelend \DIFaddbegin \DIFadd{treating all projections as equally likely regardless of how they are sampled}\DIFaddend .
...
Additionally, local building codes (based on guidance such as \citeA{asce_24-05:2006} or \citeA{FEMA_p-55:2011}) may require new construction in flood zones to be elevated \DIFaddbegin \DIFadd{a }\DIFaddend freeboard above a nominal \gls{bfe}.
...
In particular, these heuristics are scalable and explainable, they reduce the complexity of design analysis, and they are fair in at least a procedural sense. \DIFdelbegin \DIFdel{However, reliance on these heuristics also faces }\DIFdelend %DIF >  TODO: cite procedural justice?
\DIFaddbegin \DIFadd{However, these heuristics have important }\DIFaddend limitations.
...
\DIFdelbegin \DIFdel{This has }\DIFdelend \DIFaddbegin \DIFadd{These limitations have }\DIFaddend motivated economically informed approaches, like risk-based design and cost benefit analysis \cite{eijgenraam_flooding:2014,vandantzig_dike:1956,xian_elevation:2017}, that place ``a strong emphasis upon a proportionate response to risk, so that the amount invested in risk reduction is in proportion to the magnitude of the risk and the cost-effectiveness with which that risk may be reduced'' \cite{merz_fluvial:2010}.
These quantitative cost-benefit analyses also rely on probabilistic descriptions of \DIFaddbegin \DIFadd{the }\DIFaddend relevant hazard, and can be particularly sensitive to representation of tail probabilities \cite{merz_heavytails:2022,wong_floodrisk:2018,garner_slrise:2018}.
...
For example, \DIFaddbegin \DIFadd{the }\DIFaddend \gls{usgs} Bulletin 17C specifies procedures for estimating flood frequency \cite{bulletin17c:2019} and \gls{noaa} Atlas 14 provides estimates of the intensity, duration, and frequency of extreme rainfall \cite{atlas14_texas:2018}.
...
In addition, many studies formally quantify robustness to deep uncertainties and use this as a criterion for policy comparison \cite{herman:2015,mcphail_robustness:2019}.
...
This motivates the development of decision analytic frameworks that draw from the strengths of \gls{dmdu} methods such as exploratory modeling, vulnerability assessment, robustness checks, and iterative stakeholder \DIFdelbegin \DIFdel{critique}\DIFdelend \DIFaddbegin \DIFadd{input}\DIFaddend , but that embrace the reality that \DIFaddbegin \DIFadd{making }\DIFaddend assumptions about the future \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend inescapable.

\subsection{Research gaps and objectives}

We are motivated by parallels between \DIFdelbegin \DIFdel{decision making under deep uncertainty }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
    \gls{dmdu} }\hspace{0pt}%DIFAUXCMD
...
In the \DIFdelbegin \DIFdel{literature on decision making under deep uncertainty}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
    \gls{dmdu} }\hspace{0pt}%DIFAUXCMD
...
%DIF >  TODO: THIS IS THE PARAGRAPH TO ADD SOME CLARIFICATION ABOUT WHAT WE ARE AND ARE NOT DOING TO ADDERESS R2 COMMENTS / CONCERNS.
...
%DIF >  TODO: build in a little more here about why transparency matters
...
Next\DIFdelbegin \DIFdel{in \mbox{%DIFAUXCMD
...
%DIF >  TODO: add another sentence to wrap up explain the implications of the approach for the case study, for example someone elevating a house or issuing guidance / funds for house elevation

\subsection{Synthesizing deep uncertainties for decision analysis}\label{sec:results-synthesis}

The \DIFdelbegin \DIFdel{proposed approach }\DIFdelend \DIFaddbegin \DIFadd{scenario re-weighting framework described in \mbox{%DIFAUXCMD
    \cref{sec:analysis-synthesize} }\hspace{0pt}%DIFAUXCMD
...
%DIF >  TODO: add a summary sentence about how this analysis might guide decision-making
...
Yet although framing the decision through a sequential lens can increase adaptability and improve outcomes \cite{fletcher:2017,garner_slrise:2018}, \DIFdelbegin \DIFdel{the optimized policy rules are necessarily }\DIFdelend \DIFaddbegin \DIFadd{decisions and outcomes remain highly }\DIFaddend sensitive to the characterization of uncertainty \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
    \cite{herman_control:2020}}\hspace{0pt}%DIFAUXCMD
...
Additionally, since developing bespoke analyses for each house may be impractical, identifying decision rules that are applicable across different house characteristics may improve usability \DIFaddbegin \DIFadd{and guidance}\DIFaddend .
...
We develop a framework capable of blending iterative, stakeholder-driven exploratory modeling with subjective probabilistic expert assessment. %DIF >  TODO: cite ViMM Manifesto as an example of how exploratory modeling can be stakeholder-driven
