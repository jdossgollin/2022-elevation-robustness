...
%DIF DEL papers/earths-future/submission_02.tex   Wed Nov 23 10:08:47 2022
%DIF ADD papers/earths-future/submission_03.tex   Sat Dec  3 12:09:19 2022
...
}\DIFaddend For example, \citeA{sriver_sealevel:2018} sample parameters describing the rate of \gls{slr} across a range of values to inform coastal adaptation.
...
Scenario-conditional probabilistic analysis allows for uncertainty quantification and optimization, and is valuable in many \DIFdelbegin \DIFdel{copntexts}\DIFdelend \DIFaddbegin \DIFadd{contexts}\DIFaddend .
...
The four \DIFaddbegin \DIFadd{physical process }\DIFaddend models considered are (i) the BRICK model (version 0.2) with slow (``BRICK Slow'') and (ii) fast (``BRICK Fast'') ice sheet dynamics \cite{wong_brick0.2:2017}, (iii) the \citeA{kopp_probabilistic:2014} model (``K14''), and (iv) the \citeA{deconto_antarctica:2016} model (``DP16'').
...
We return in \cref{sec:results-conditional} to the challenge of decision making given multiple \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\glspl{pdf}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{scenarios}\DIFaddend .
...
\subsection{\DIFdelbegin \DIFdel{Prior over }\DIFdelend \DIFaddbegin \DIFadd{Subjective probability distributions for }\DIFaddend \glsentrylong{slr}}\label{sec:case-priors}
...
We use a Gamma distribution for all three \DIFdelbegin \DIFdel{priors}\DIFdelend \DIFaddbegin \DIFadd{probability distributions}\DIFaddend , parameterized following eq.~(S4).
...
We developed these \DIFdelbegin \DIFdel{priors }\DIFdelend \DIFaddbegin \DIFadd{subjective distributions }\DIFaddend for didactic purposes, to illustrate a range of possible beliefs.
...
\begin{figure}
  ...
  \caption{
    ...
    Light gray lines show estimates for all 16 models (four \gls{rcp} scenarios \DIFdelbeginFL \DIFdelFL{times }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\times$ }\DIFaddendFL four physical \DIFdelbeginFL \DIFdelFL{parameterizations}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{process models}\DIFaddendFL ) considered.
...
This approach is, in a sense, another form of exploratory modeling: instead of considering a very large ensemble of \glspl{sow}, we consider a much smaller set of probabilistic \DIFdelbegin \DIFdel{models}\DIFdelend \DIFaddbegin \DIFadd{scenarios}\DIFaddend .
...
Although this scenario-conditional analysis is appropriate for understanding differences between models, its key limitation is that it \textbf{places the burden for deciding which \DIFdelbegin \DIFdel{model }\DIFdelend \DIFaddbegin \DIFadd{probabilistic scenario }\DIFaddend to design for onto the end user.}
