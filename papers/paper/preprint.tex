...
Specifically, we use experiments using the BRICK model version 0.2 \citep{wong_brick0.2:2017} simulations published in \citet{ruckart_hazardous:2008}.
There are eight \glspl{pdf}, each with ZXX\james{count} simulations: four emissions pathways (\gls{rcp} scenarios) $\times$ two models of ice sheet dynamics.

For consistency with literature on decision making under deep uncertainty \citep[see][]{marchau:2019}, we refer to one time series of \gls{slr} ($\overline{y}_t$) as \emph{a \acrfull{sow}}.
...
\begin{figure}
    ...
    \caption{
        ...
        (b):
        return periods.
        Dots indicate observed values; their $x$-value (``plotting position'') is calculated using the Weibull rule.
...
Following \citet{garner_slrise:2018}, \citet{oddo_coastal:2017}, and \citet{lempert_slr:2012} we place a prior on \gls{slr} in 2100.\klaus{You suggest looking at another paper. Are you referring to this?\\\fullcite{sriver_sealevel:2018}}
