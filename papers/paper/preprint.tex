...
These trigger specific floodplain regulations, such as a nominal requirement for homes purchased with federally backed mortgages to be covered by flood insurance \citep{kousky_voucher:2014}.
Additionally, local building codes \citep[based on guidance such as][]{asce_24-05:2006,FEMA_p-55:2011}, may require new construction in flood zones to be elevated some margin above a nominal \gls{bfe}.
Design standards are also used to design large-scale infrastructure.
For example, some levees in the Netherlands are required to be designed such that the annual probability of failure is less than 1 in \num{4000} \citep{eijgenraam_flooding:2014}, while a seawall proposed as part of a \$29 billion coastal protection project for  Galveston Bay was designed for the annual probability of overtopping to equal 1\%, neglecting the effects of sea level rise \citep[Appendix D.,~p.~2-59]{USACE_coastal:2021}.

There are many advantages to standards-based design.
In particular, these heuristics are scalable and explainable, do not require complex analysis to apply, and are fair in at least a procedural sense.\klaus{I understand this to mean that they are the same for everyone, even if they don't necessarily lead to equitable outcomes -- is that how you read this?}
...
Like standards-based approaches, quantitative cost-benefit analyses rely on probabilistic descriptions of relevant hazard.
...
Although some methods have been proposed for incorporating trends into these analyses \citep[see][for a review]{Salas:2018ge}, these assume specific forms of nonstationarity which may not fully represent physical processes or true levels of uncertainty \citep{DossGollin:2019,Montanari:2014hl,Serinaldi:2015bq}.
...
This construct is a theoretical contrast to the ``$\mathcal{M}$-closed'' case, where the goal is to select the ``true'' model from a candidate space.
...
Similarly, we assume that a fixed ensemble of \glspl{sow} are available for analysis, but in some cases additional simulations may add value.
...
Exploratory modeling avoids making explicit assumption about the likelihood of different \glspl{sow} and instead seeks to generate new knowledge \citep{bankes:1993}, for example by discovering relevant scenarios  \citep{lamontagne_discovery:2018,groves_scenarios:2007} or mapping a system's response to forcing \citep{Poff:2015jn,Steinschneider:2015kk,sriver_sealevel:2018}.
...
For example, answering questions like ``what is the 95th percentile of metric $u$ under decision $x$'' or ``which decision minimizes expected damages'' or ``what is the probability of exceeding a critical threshold'' requires estimating the expectation of some function $\pi$, which has the form
\begin{equation}\label{eq:expectation-is-probability}
    \mathbb{E}\qty[f(s | x_i)] = \int_{s \in \Omega} p(s) \pi(s | x_i) \dd{s}.
\end{equation}
In other words, \cref{eq:expectation-is-probability} shows that \emph{estimating any such expectation requires defining a probability distribution over \glspl{sow}}, $p(s)$.
...
If the \glspl{sow} are drawn \gls{iid} from $M_k$, the set of outcomes $u_{i, j}$ can be interpreted as \gls{iid} draws from the conditional distribution over outcomes, $p(u | x_i, M_k)$, allowing Monte Carlo estimation of \cref{eq:expectation-is-probability} \citep{betancourt:2018}.

This framework can be applied to understand many existing analyses.
For example, if we define $M$ to be a probabilistic scenario (\eg, a choice of physical model and of an \gls{rcp} scenario, as we shall see in the next section), then we can rigorously quantify uncertainty \emph{within models}, but can only characterize uncertainty \emph{between models} \citep{wong_nola:2017,ruckert_coastal:2019,sharma_rcp:2021}.
...
For example, if this product of independent Uniform distributions truly represents a well-calibrated belief about the joint probability distribution over \glspl{sow} $p(\Omega)$ then it should be used, but analysts should be clear that, like it or not, they are making probabilistic assumptions, and should apply methods for model critique and validation \citep[see][]{gelman_workflow:2020}.
...
This weighting approach parallels the joint probability method used in analysis of tropical cyclones \citep{johnson_clara:2013,resio_probabilities:2007,toro_jpm-os:2010}.
...
One application of explotatory analysis is to reveal uncertainty and variance in outcome under particular decisions.
