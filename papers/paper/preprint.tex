...
\Cref{fig:lsl-evolution} shows the \glspl{pdf} of mean sea level in 2100 under each of the eight models considered: four emissions pathways \citep[\gls{rcp} scenarios][]{moss_rcp:2010} and two models of ice sheet dynamics.
...
\begin{figure}
    ...
    \caption{
        ...
        Gray lines show the 50, 80, and 95\% posterior confidence intervals from the Bayesian \gls{gev} fit.
...
An alternative interpretation of the available \glspl{sow} is as \gls{iid} draws from each of eight \glspl{pdf} of sea level rise, as shown in \cref{fig:lsl-evolution}.
...
For example, \cref{fig:tradeoffs-by-rcp}(a) plots the expected lifetime cost\james{update figure} as a function of $\Delta h$\james{Add $\Delta h$ to the plot} for four of the eight models available.
Similarly, \cref{fig:tradeoffs-by-rcp} plots the 80\% confidence interval width (\ie, the 90th percentile minus the 10th percentile) of lifetime cost.
...
Calculating either of the quantities shown in \cref{fig:tradeoffs-by-rcp} \emph{requires synthesizing across \glspl{sow}}.
...
We introduce an approach to bridge this divide through a computationally efficient method that weights each \gls{sow} based on a probability distribution over possible futures.
